...
In residential neighbourhoods, various measures have been shown to improve road safety and raise cycling levels, such as the promotion of 'liveable streets' - where the emphasis is on reducing vehicle speeds and volumes (TfL 2019), 'filtered permeability' - where through traffic is allowed for cycling and walking, but not driving (Melia 2015), and the construction of segregated or on-road cycle routes (Aldred et al. 2017). But where can these techniques be applied to best effect? We aim to provide the tools that planning officers, consultants or others can use to determine where the best locations are for these measures to be put in place, and the evidence that can be used to support their introduction.
...
To do this, we must first locate the bounds of residential zones then look at where these zones might be served by major safe routes to schools. Residential zones are identified using built-up areas as defined in ONS data (ref), then further refined using distance from LSOA centroids.
...
Pezzagno, Michèle, and Maurizio Tira. Town and Infrastructure Planning for Safety and Urban Quality: Proceedings of the XXIII International Conference on Living and Walking in Cities (LWC 2017), June 15-16, 2017, Brescia, Italy. CRC Press, 2018.
